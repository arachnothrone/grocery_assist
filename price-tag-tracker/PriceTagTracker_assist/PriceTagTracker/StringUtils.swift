...
extension String {
	...
	func extractPhoneNumber() -> (Range<String.Index>, String)? {
        ...
        let pattern2 = #"""
        (?x)                    # Verbose regex, allows comments
        \b(\w{1,})              # One or more digits (or letter characters e.g. 'S' -> '5') whole part of the number
        ([.|,])                 # Decimal point
        (\w{2})\b               # Two digits/letters - cents
        """#
		
		guard let range = self.range(of: pattern2, options: .regularExpression, range: nil, locale: nil) else {
